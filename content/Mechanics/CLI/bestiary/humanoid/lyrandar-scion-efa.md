---
publish: true
aliases: Lyrandar Scion
created: 2026-03-15T01:10:08.601+01:00
modified: 2026-03-15T01:08:43.886+01:00
tags:
  - ttrpg-cli/compendium/src/5e/efa
  - ttrpg-cli/monster/cr/4
  - ttrpg-cli/monster/size/small-or-medium
  - ttrpg-cli/monster/type/humanoid/khoravar
cssclasses: json5e-monster
---

# Lyrandar Scion
*Source: Eberron: Forge of the Artificer p. 82*  
![](Mechanics/CLI/bestiary/humanoid/img/lyrandar-scion.webp#right)

The Khoravar dragonmarked scions of House Lyrandar include swashbuckling sky captains, seafaring adventurers, and sinister cultists of the Hurricane Harvest. The power of their dragonmarks manifests as easy control of wind, thunder, and lightning, which they wield as readily in combat as in navigating the sea and sky.

Members of the Hurricane Harvest believe that the Mark of Storm is not a gift of the Sovereign Host, as others in their house teach, but the sign of the Devourer. They maintain that the greatest ancestors of their house live on as krakens in the deep and guide the cult through visions to use their power for destruction and domination. They sometimes partner with kuo-toa, sahuagin, and other monsters of the deep.

```ad-statblock
title: Lyrandar Scion
![](Mechanics/CLI/bestiary/humanoid/token/lyrandar-scion-efa.webp#token)
*Small or Medium humanoid (khoravar), Neutral*

- **Armor Class** 13 
- **Hit Points** 60 (`11d8 + 11`) 
- **Speed** 30 ft.

|STR|DEX|CON|INT|WIS|CHA|
|:---:|:---:|:---:|:---:|:---:|:---:|
|12 (+1)|16 (+3)|13 (+1)|12 (+1)|15 (+2)|18 (+4)|

- **Proficiency Bonus** +2
- **Saving Throws** Strength +3, Dexterity +5
- **Skills** [Acrobatics](Mechanics/CLI/rules/skills.md#Acrobatics) +5, [Perception](Mechanics/CLI/rules/skills.md#Perception) +4
- **Senses** [Darkvision](Mechanics/CLI/rules/senses.md#Darkvision) 60 ft., passive Perception 14
- **Damage Resistances** lightning, thunder
- **Gear** [wand](Mechanics/CLI/items/wand-xphb.md)
- **Languages** Common, Elvish
- **Challenge** 4

## Actions

***Multiattack.*** The scion makes two Thunderbolt attacks and uses Spellcasting.

***Thunderbolt.*** *Melee  or Ranged Attack Roll:* `+6`, reach 5 ft. or range 30 ft. *Hit:* 11 (`2d6 + 4`) Lightning damage plus 9 (`2d8`) Thunder damage. Critical *Hit:* The creature also has the [Deafened](Mechanics/CLI/rules/conditions.md#Deafened) condition for 1 minute.

***Spellcasting.*** The scion casts one of the following spells, using Charisma as the spellcasting ability (spell save DC 14):

**At will:** [Elementalism](Mechanics/CLI/spells/elementalism-xphb.md), [Gust of Wind](Mechanics/CLI/spells/gust-of-wind-xphb.md), [Thunderwave](Mechanics/CLI/spells/thunderwave-xphb.md)

## Bonus Actions

***Jump (2/Day).*** The scion casts [Jump](Mechanics/CLI/spells/jump-xphb.md), requiring no spell components and using the same spellcasting ability as Spellcasting.


## Reactions

***Feather Fall (2/Day).*** The scion casts [Feather Fall](Mechanics/CLI/spells/feather-fall-xphb.md) in response to that spell's trigger, using the same spellcasting ability as Spellcasting.

```
^statblock